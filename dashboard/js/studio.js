...
(function () {
  ...
  const API = '';  // same origin — server serves both dashboard and API
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Hook into showPage so profiles load when studio tab opens
    const _origShow = window.showPage;
    window.showPage = function (page) {
      _origShow(page);
      if (page === 'studio') loadStyleProfiles();
    };
  });
  ...
  function initAnalyzeBtn() {
    analyzeBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        clearInterval(ticker);
        analyzeLoading.style.display = 'none';
        analyzeResult.innerHTML = `<div class="studio-empty" style="color:var(--red-lt)">Error: ${esc(err.message)}</div>`;
        analyzeResult.style.display = 'block';
      } finally {
  ...
  async function loadStyleProfiles() {
    ...
    try {
      ...
      // Render summary cards + one shared expanded panel
      profilesGrid.innerHTML = `
      ...
      list.forEach((profile, i) => {
        ...
        if (card) card.addEventListener('click', (e) => {
          // Don't open panel when clicking inside the rename UI
          if (e.target.closest('.studio-rename-btn, .studio-rename-save, .studio-rename-input')) return;
          toggleProfilePanel(profile, i, list.length);
        });

        const renameBtnEl = document.getElementById(`studio-rename-${i}`);
        if (renameBtnEl) renameBtnEl.addEventListener('click', (e) => {
          e.stopPropagation();
          startRename(i, profile);
        });
      });
  ...
  function startRename(i, profile) {
    ...
    titleRow.innerHTML = `
      <input class="studio-rename-input" id="studio-rename-input-${i}"
             value="${esc(currentTitle)}" maxlength="80">
      ...
      fetch(`${API}/api/style-profiles/${encodeURIComponent(profile.filename)}`, {
        ...
        .then(data => {
          if (data.success) {
            ...
            titleRow.innerHTML = `
              <div class="studio-psummary-title" id="studio-title-display-${i}">${esc(data.title)}</div>
              <button class="studio-rename-btn" id="studio-rename-${i}" title="Rename">&#9998;</button>
            `;
            document.getElementById(`studio-rename-${i}`)
              .addEventListener('click', (e) => { e.stopPropagation(); startRename(i, profile); });
            // Update open panel header if this profile is expanded
            if (_openPanelIdx === i) {
              const panelTitle = document.querySelector('.studio-panel-title');
    ...
    function cancel() {
      titleRow.innerHTML = `
        <div class="studio-psummary-title" id="studio-title-display-${i}">${esc(currentTitle)}</div>
        <button class="studio-rename-btn" id="studio-rename-${i}" title="Rename">&#9998;</button>
      `;
      document.getElementById(`studio-rename-${i}`)
        .addEventListener('click', (e) => { e.stopPropagation(); startRename(i, profile); });
  ...
  function toggleProfilePanel(profile, i, _total) {
  ...
  function initMasterBtn() {
    masterBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        masterLoading.style.display = 'none';
        masterCards.innerHTML = `<div class="studio-empty" style="color:var(--red-lt)">Error: ${esc(err.message)}</div>`;
        masterWrapper.style.display = 'block';
      } finally {
  ...
  // Column 1: Hook + Caption Style
  // Column 2: Text Overlays
  // Column 3: Zooms + Audio
  // Full-width bottom: Pacing + Style Fingerprint
  function renderFullWidth(data) {
    ...
    if (Array.isArray(data.zooms)) {
      ...
      col3.push(card('Zooms', items.length
        ? `<div class="studio-timeline">${items.map(z => tlItem(z.timestamp,
            [z.direction, z.speed].filter(Boolean).join(' \u00b7'),
            z.subject
          )).join('')}</div>`
        : '<span class="studio-tl-sub">None detected</span>'));
    }
    ...
    if (data.pacing) {
      const p    = data.pacing;
      const cuts = Array.isArray(p.cut_timestamps) ? p.cut_timestamps : [];
      bottom.push(card('Pacing',
        kv('Overall', p.overall) +
        kv('Feel', p.rhythm_description) +
    ...
    if (data.style_fingerprint) {
      bottom.push(card('Style Fingerprint',
        `<p class="studio-fingerprint-text">${esc(data.style_fingerprint)}</p>`,
        'full'
      ));
    }

    // Fallback for master guide / unexpected shapes
    if (!col1.length && !col2.length && !col3.length && !bottom.length) {
      bottom.push(card('Style Guide',
        Object.entries(data)
          .filter(([k]) => k !== 'title')
          .map(([k, v]) => kv(k, typeof v === 'object' ? JSON.stringify(v) : String(v))).join(''),
        'full'
      ));
    }

    const hasColumns = col1.length || col2.length || col3.length;
    const colHtml = hasColumns ? `
      <div class="studio-layout-3col">
    ...
    const bottomHtml = bottom.length
      ? `<div class="studio-layout-bottom">${bottom.join('')}</div>`
      : '';

    return colHtml + bottomHtml;
  }

  // ── Primitives ────────────────────────────────────────────────────────────
  function card(title, body, width) {
    const cls = width === 'full' ? ' full-width' : '';
    return `<div class="studio-card${cls}">
  ...
  function kv(key, val) {
    if (!val && val !== 0) return '';
    const valHtml = (typeof val === 'string' && /^</.test(val)) ? val : esc(String(val));
    return `<div class="studio-kv"><span class="studio-kv-key">${esc(String(key))}</span><span class="studio-kv-val">${valHtml}</span></div>`;
  }
  ...
  function sortByTs(arr) {
    return [...arr].sort((a, b) => ts2s(a.timestamp) - ts2s(b.timestamp));
  }
  ...
  function initReelGenerator() {
    ...
    reelDownloadBtn.addEventListener('click', async () => {
      ...
      if (!/^[a-z0-9_-]+$/i.test(project)) {
        showReelError('Project name may only contain letters, numbers, hyphens, and underscores.');
        return;
      }
  ...
  function renderClipsList(clips) {
    ...
    reelClipsList.innerHTML = `
      <div class="reel-clips-table">
        <div class="reel-clips-header">
          <span>Filename</span><span>Size</span><span>Duration</span>
        </div>
        ${clips.map(c => `
  ...
  function showReelError(msg) {
    reelError.textContent   = msg;
    reelError.style.display = 'block';
  }
